...
The live contract below describes the next release and is verified against `src/quarto_needs/surface.py`. Published release contracts are frozen separately under `release-surfaces/` and must not be regenerated from the live registry.
...
- Migration extras are authored. Scalar source-specific attributes (`priority`, `owner`, numbers, booleans) become preamble attributes in the rendered `.need` block, a `tags` value embedded in a ReqIF export's attributes payload is recovered as tags, and update detection now reports an `attributes` change when an upstream extra changes. Values the grammar cannot represent — lists, mappings, multi-line text, or a name reserved by the grammar or the relation catalog — stay in the plan's `extras` for review instead of being silently altered or dropped.
...
Frozen from `release-surfaces/0.1.1.json`, anchored to tag `v0.1.1` at commit `d4247bfd9a170e1160706967178e90304766a8a6`.
