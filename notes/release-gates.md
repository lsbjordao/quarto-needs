...
| Migrations | Every source declared in `migration_cli.SOURCES` converges on one plan → apply-plan/update-plan → reviewed write contract, with rollback and refusal-based idempotence (`sphinx-needs`, `doorstop`, `strictdoc`, `openfasttrace`, `reqif`) | `tests/test_migration_*.py`, `tests/test_sphinx_needs_migration.py`, `tests/test_doorstop_migration*.py`, `tests/test_strictdoc_migration*.py`, `tests/test_openfasttrace_migration.py`, `tests/test_reqif_migration*.py` | core CI |
