...
def run_oslc_action(root: Path, argv: Sequence[str], action: str) -> int:
    if action not in {"discover", "catalog"}:
        if not action:
            print("OSLC action required: discover or catalog", file=sys.stderr)
        else:
            print(f"Unknown OSLC action: {action}", file=sys.stderr)
        return 2

    try:
        return _run_discover(root, argv) if action == "discover" else _run_catalog(root, argv)
