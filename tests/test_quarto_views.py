...
@pytest.mark.skipif(shutil.which("quarto") is None, reason="Quarto is not installed")
def test_need_c4_renders_a_dynamic_diagram(tmp_path: Path):
    """The dynamic level renders a real C4Dynamic sequence, not source."""
    project = build_c4_fixture_project(tmp_path)
    assert (
        project / ".quarto-needs" / "graphs" / "c4-dynamic-SYS-1.json"
    ).is_file()
    subprocess.run(
        ["quarto", "render", str(project)],
        cwd=ROOT,
    ...
    assert "C4Dynamic" not in html
    assert "starts a render" in html
    assert "reports progress" in html
