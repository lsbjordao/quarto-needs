...
local function render_mermaid_png(source)
  local digest=pandoc.utils.sha1(source); local image_name="quarto-needs-flow-"..digest..".png"; if rendered_flow_images[digest] then pandoc.mediabag.insert(image_name,"image/png",rendered_flow_images[digest]); return image_name end
  local ok,result=pcall(pandoc.system.with_temporary_directory,"quarto-needs-flow",function(directory) local input=pandoc.path.join({directory,"flow.qmd"}); local output=io.open(input,"wb"); if not output then return {error="temp source"} end; output:write("---\nmermaid-format: png\nformat: html\n---\n\n```{mermaid}\n",source,"\n```\n"); output:close(); local rendered=pandoc.system.with_working_directory(directory,function() return pcall(pandoc.pipe,quarto.config.cli_path(),{"render","flow.qmd","--to","html","--output","flow.html"},"") end); if not rendered then return {error="render failed"} end; local image_path=pandoc.path.join({directory,"flow_files","figure-html","mermaid-figure-1.png"}); local image=io.open(image_path,"rb"); if not image then return {error="no png"} end; local contents=image:read("*a"); image:close(); return {contents=contents} end)
  if not ok or result.error then return nil,result.error or pandoc.utils.stringify(result) end; pandoc.mediabag.insert(image_name,"image/png",result.contents); rendered_flow_images[digest]=result.contents; return image_name
end
local function render_need_flow(args,kwargs)
  ...
  local blocks={}; for _,message in ipairs(plan.warnings) do blocks[#blocks+1]=pandoc.Div({pandoc.Para({pandoc.Str(message)})},pandoc.Attr("",{"need-view-warning"},{role="status"})) end; local image_name,render_error=render_mermaid_png(plan.source); if not image_name then quarto.log.warning(render_error or "flow render failed"); return views.warning(L("Could not render this need-flow diagram.","Não foi possível renderizar este diagrama de fluxo.")) end
  local description=L("Traceability flow with ","Fluxo de rastreabilidade com ")..tostring(plan.node_count)..L(" needs"," objetos"); local image=pandoc.Image({pandoc.Str(description)},image_name,"",pandoc.Attr("",{"need-flow"},{role="img"})); blocks[#blocks+1]=pandoc.Div({pandoc.Para({image})},pandoc.Attr("",{"need-flow-scroll"})); return pandoc.Div(blocks)
end
