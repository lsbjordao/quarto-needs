...
The envelope is intentionally provider-neutral. Pytest is the first executable provider, but the same attestation contract is designed to wrap JUnit XML normalization, coverage results, Quarto render checks, JSON Schema validation, and lint/type-check providers without changing the canonical engineering graph.
...
Generation is only half of the contract. Validate either a raw pytest artifact or an attested pytest envelope against the canonical engineering graph:
...
The existing command remains backward compatible with raw `evidence-pytest-v1`. When the artifact is an attestation envelope, the same command additionally checks its provenance against the current snapshot.

The semantic check verifies that:
...
- an expected source revision, when supplied by a caller, agrees with the attested revision.
...
The self-hosted Quarto-Needs case study exercises executable evidence directly:
...
The raw pytest payload remains supported during the transition to attested self-hosted evidence. The provider-neutral envelope is the target contract for new machine-evidence providers and for freshness-sensitive CI usage.

`make render-self-example` depends on the evidence target, so the example cannot render successfully with inconsistent executable bindings.
