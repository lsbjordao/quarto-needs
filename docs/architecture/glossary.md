...
: Authored artifacts are `.qmd` declarations, `.quarto-needs.toml`, and Markdown prose — reviewed and versioned as source. Generated artifacts (`.quarto-needs/needs.json`, `.quarto-needs/graphs/*.json`, `generated-index.lua`, everything under `docs/architecture/generated/`) are rebuilt on every run and never hand-edited (`ADR-017`).
