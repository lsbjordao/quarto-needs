...
`EVD306` and `EVD309` are the generic counterpart of the reciprocal gate described in [Executable evidence](executable-evidence.qmd): a provider may claim a requirement only when the model independently links that requirement to the claimed test-case, and only when the modeled evidence object is itself connected to it.
