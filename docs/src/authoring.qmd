...
A relation value may carry inline attributes after its targets: `name=value`, with a quoted value when it contains spaces. Attributes on one value apply to every target on that value; use the list form for targets that need different attributes. The C4 projections read `label` as the relationship description, `technology` as the relationship technology, and `order` as the sequence number of an `interacts-with` interaction in a dynamic view:
...
Unknown keys remain valid custom attributes. To catch the narrow case where an intended relation was probably misspelled, validation reports `QND005` when an unknown key is exactly one insertion, deletion, substitution, or adjacent transposition away from a real authored relation name. For example, `verifed-by: TC-020` warns that it resembles `verified-by`. The warning never rewrites the key and never creates an edge: the original value remains an ordinary attribute until the author fixes it. Names outside that one-edit radius stay silent, preserving the open custom-attribute namespace.
...
Relation targets are not resolved while parsing. A target that names no declared object is reported as `REQ005` during validation. Relation-name near misses are the separate `QND005` advisory above; neither diagnostic silently repairs authored data.
