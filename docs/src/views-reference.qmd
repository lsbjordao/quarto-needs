...
Examples below assume the referenced IDs exist. A `query` names a query declared in `.quarto-needs.toml`, rather than an expression written inside the shortcode. See [Named queries](named-queries.qmd) for the complete query grammar. Views render object titles in the page's language when localized title siblings exist; see [Localized titles](authoring.qmd#localized-titles).
