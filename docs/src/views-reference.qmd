...
Examples below assume the referenced IDs exist. A `query` names a query declared in `.quarto-needs.toml`, rather than an expression written inside the shortcode. See [Engineering model](engineering-model.qmd) for a complete query declaration. Views render object titles in the page's language when localized title siblings exist; see [Localized titles](authoring.qmd#localized-titles).
