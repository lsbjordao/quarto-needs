...
function M.get(graph, id) return data.get(graph, id) end
function M.outgoing(graph, id, relation_type) return data.outgoing(graph, id, relation_type) end
function M.incoming(graph, id, relation_type) return data.incoming(graph, id, relation_type) end
...
function M.is_html_format() return detect_format() == "html" end
...
local function normalize_mermaid_svg(html)
  ...
  if not width then width, height = open_tag:match("viewBox%s*=%s*'[%d%.%-]+%s+[%d%.%-]+%s+([%d%.%-]+)%s+([%d%.%-]+)'") end
...
local function with_temp_mermaid(source, temp_name, mermaid_format, collect)
  ...
  if type(result) ~= "table" or result.error then return nil, (type(result) == "table" and result.error) or "render failed" end
  return result.data
end
...
local function collect_temp_html(directory)
  ...
  local html = f:read("*a"); f:close(); return {data = html}
end
...
local function collect_temp_png(directory)
  ...
  local contents = f:read("*a"); f:close(); return {data = contents}
end
...
function M.mermaid_inline_svg(source, description, class_name)
  ...
  if class_name and class_name ~= "" then
    if open_tag:find('class="') then open_tag = open_tag:gsub('class="', 'class="' .. class_name .. " ", 1)
    else open_tag = open_tag .. ' class="' .. class_name .. '"' end
  end
  open_tag = open_tag:gsub('%s*role="[^"]*"', "")
  local extras = ' role="img"'
  if description and description ~= "" then extras = extras .. ' aria-label="' .. escape_html_attr(description) .. '"' end
...
function M.render_mermaid_asset(source, prefix)
  ...
  if cached then pandoc.mediabag.insert(cached.name, "image/png", cached.data); return cached.name end
...
local function current_input()
  ...
  if ok and type(declared) == "string" and declared ~= "" then input = declared
  else input = PANDOC_STATE and PANDOC_STATE.input_files and PANDOC_STATE.input_files[1] end
...
local function option_values(kwargs, ...)
  ...
  for n = 1, select("#", ...) do
    ...
    for value in raw:gmatch("[^,;]+") do value = value:match("^%s*(.-)%s*$"); if value ~= "" then values[value:lower()] = true end end
  end
  return values
end

local function attributes(object) return type(object.attributes) == "table" and object.attributes or {} end
local function matches(value, options) if next(options) == nil then return true end; return options[text(value):lower()] == true end
local function matches_tags(value, options)
  if next(options) == nil then return true end
  if type(value) == "table" then for _, tag in ipairs(value) do if matches(tag, options) then return true end end; return false end
  for tag in text(value):gmatch("[^,;]+") do if matches(tag:match("^%s*(.-)%s*$"), options) then return true end end
  return false
end

function M.filter(objects, kwargs)
  local ids = option_values(kwargs, "ids"), types = option_values(kwargs, "types", "type"), statuses = option_values(kwargs, "status"), priorities = option_values(kwargs, "priority"), tags = option_values(kwargs, "tags")
  local result = {}
  for _, object in ipairs(objects or {}) do
    local attrs = attributes(object)
    if matches(object.id, ids) and matches(object.type, types) and matches(object.status, statuses) and matches(attrs.priority, priorities) and matches_tags(attrs.tags, tags) then table.insert(result, object) end
  end
  return result
end
...
function M.named_query_ids(graph, name)
  ...
  local wanted = {}; for _, id in ipairs(ids) do wanted[text(id)] = true end; return wanted
end

function M.select(graph, objects, kwargs)
  local query_name = M.kwarg(kwargs, "query"), pool = objects
  if query_name ~= "" then
    local wanted = M.named_query_ids(graph, query_name)
    if wanted == nil then return {}, "Unknown query: " .. query_name end
    pool = {}; for _, object in ipairs(objects or {}) do if wanted[text(object.id)] then pool[#pool + 1] = object end end
  end
...
function M.sort(objects, kwargs)
  local fields = {}; for value in M.kwarg(kwargs, "sort"):gmatch("[^,;]+") do table.insert(fields, value:match("^%s*(.-)%s*$"):lower()) end
  table.sort(objects, function(a, b)
    for _, field in ipairs(fields) do
      local av, bv
      if field == "priority" then av = priority_order[text(attributes(a).priority):lower()] or 5; bv = priority_order[text(attributes(b).priority):lower()] or 5
      else av = text(a[field] or attributes(a)[field]):lower(); bv = text(b[field] or attributes(b)[field]):lower() end
...
local generated_ids, reserved_view_ids = {}, {}
local function safe_view_id(value, fallback)
  local normalized = M.slug(value); if normalized == "" then normalized = M.slug(fallback) end; if normalized == "" then normalized = "need-view" end; return normalized
end
function M.reserve_view_id(prefix, requested)
  local safe_prefix = safe_view_id(prefix, "need-view"), base
  if text(requested) == "" then generated_ids[safe_prefix] = (generated_ids[safe_prefix] or 0) + 1; base = safe_prefix .. "-" .. tostring(generated_ids[safe_prefix])
  else base = safe_view_id(requested, safe_prefix) end
  local candidate, suffix = base, 2
  while reserved_view_ids[candidate] do candidate = base .. "-" .. tostring(suffix); suffix = suffix + 1 end
  reserved_view_ids[candidate] = true; return candidate
end
function M.next_id(prefix) return M.reserve_view_id(prefix) end

function M.badge(kind, value)
  local raw = text(value); if raw == "" then return {} end
...
function M.link(object, label, options)
  local resolved = {format = options and options.format or detect_format(), current_input = options and options.current_input or current_input()}
  return pandoc.Link({pandoc.Str(label or text(object.id))}, data.link_target(object, resolved), "")
end
function M.objects_by_id(objects) local result = {}; for _, object in ipairs(objects or {}) do result[text(object.id)] = object end; return result end
function M.related(graph, source, relation_type) local result = {}; for _, relation in ipairs(data.outgoing(graph, source, relation_type)) do table.insert(result, relation.target) end; return result end
function M.table(caption, headers, rows, attr)
  local function blocks(cells) local result = {}; for _, cell in ipairs(cells) do result[#result + 1] = pandoc.Plain(cell) end; return result end
  local normalized_rows = {}; for _, row in ipairs(rows) do normalized_rows[#normalized_rows + 1] = blocks(row) end
  local aligns, widths = {}, {}; for _ = 1, #headers do aligns[#aligns + 1] = "AlignDefault"; widths[#widths + 1] = 0 end
  local simple = pandoc.SimpleTable({pandoc.Str(caption or "")}, aligns, widths, blocks(headers), normalized_rows)
  local table_block = pandoc.utils.from_simple_table(simple); table_block.attr = attr or pandoc.Attr(); return table_block
end
function M.node_id(id) return "need_" .. text(id):gsub(".", function(char) return string.format("%02X", string.byte(char)) end) end
function M.escape_mermaid(value)
  return text(value):gsub("[\"\\\n\r]", function(char) if char == "\"" then return "'" end; if char == "\\" then return "/" end; return " " end)
end
