(() => {
  ...
  function enhance(container) {
    ...
    const showContext = () => {
      ...
      if (parents.input.checked) semanticRelatives(cy, selectedNodeId, "parents").forEach((id) => visible.add(id));
      if (children.input.checked) semanticRelatives(cy, selectedNodeId, "children").forEach((id) => visible.add(id));
      cy.nodes().forEach((node) => node.style("display", visible.has(node.id()) ? "element" : "none"));
      cy.edges().forEach((edge) => {
    ...
    search.addEventListener("input", () => { selectedNodeId = null; });
