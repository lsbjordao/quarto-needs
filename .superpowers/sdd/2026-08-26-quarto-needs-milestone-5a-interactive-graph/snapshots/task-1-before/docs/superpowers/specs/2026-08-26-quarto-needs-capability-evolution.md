...
| 3 | 5A — Interactive graph | The visible leap. It is what gets shown, shared, and remembered — and it makes the Milestone 3 change intelligence legible instead of theoretical. |
| 4 | 4B — Source adapters | Requirements reaching real code and tests. This is what separates an engineering tool from a document tool, and it is the gap against OpenFastTrace and StrictDoc. |
...
**5A and 4B were swapped on 2026-08-26, by the project owner's decision.** The
original order put source adapters first so the graph would show traceability
into code rather than redraw links the project already had. That argument is
about the graph's *completeness*; it was outweighed by what the graph is for
right now. With the repository still private, nobody outside the project can
install anything, which removes the adoption pressure that justified deferring
the visible work — and a graph that renders requirements, tests, components and
risks, with Milestone 3's diff and impact overlaid, is already a demonstration
worth having. 4B then makes it complete rather than making it exist.

The cost of this swap, stated plainly: the first interactive graph will show
authored relationships only. Code and test nodes arrive with 4B, and the graph
will need a second pass to display them.
