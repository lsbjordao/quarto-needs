...
def query_ids(config: object, snapshot: AnalysisSnapshot, name: str) -> tuple[str, ...]:
    """Evaluate one named query against the snapshot.

    The built-in `approved-requirements` query is available under its own name
    unless a user query overrides it, mirroring `materialize_queries`. There is
    no other grammar: the graph reuses the safe query evaluator and never
    interprets query text anywhere downstream.
    """
    if not isinstance(name, str) or not name.strip():
        raise QueryError("query name must be a non-empty string")
    sources: Mapping[str, object] = getattr(config, "named_query_sources", {}) or {}
    if name in sources:
        raw: object = sources[name]
    elif name == DEFAULT_QUERY_NAME:
        raw = dict(DEFAULT_QUERY_SOURCE)
    else:
        known = ", ".join([DEFAULT_QUERY_NAME, *sorted(sources)])
        raise QueryError(f"no named query {name!r} (known: {known})")
    query = compile_query(name, raw if isinstance(raw, Mapping) else {})
    return tuple(record.id for record in evaluate(query, snapshot))
